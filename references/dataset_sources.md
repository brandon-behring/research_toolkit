# Dataset sources catalog (v1.6)

Reference doc for `/dataset-gather`. Catalogs the 8 source categories the skill
...
4. Aggregators (paperswithcode, Google Dataset Search; cross-vendor)
5. Domain-specific portals (NIH, FRED, NASA, Common Crawl)
...
- **URL pattern**: `https://paperswithcode.com/datasets?q=<topic>`
- **Discovery**: Their search is good. Each dataset has a list of papers using it.
- **Metadata extraction**: License, modality, task tags. Cross-references to papers.
- **Gotchas**: Many entries are stubs (link out to HF/GitHub/etc.). Use the link-out as `primary_url`.
